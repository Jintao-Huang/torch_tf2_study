...
def main():
    ...
    x = torch.linspace(-1, 1, 1000, device='cuda:0')[:, None]  # shape(1000, 1)
    y_true = x ** 2 + 2 + torch.normal(0, 0.1, x.shape, device='cuda:0')

    # 2. 参数初始化
    w1 = torch.normal(0, 0.1, (1, hide_c), requires_grad=True, device='cuda:0')
    w2 = torch.normal(0, 0.1, (hide_c, 1), requires_grad=True, device='cuda:0')
    b1 = torch.zeros((hide_c,), requires_grad=True, device='cuda:0')
    b2 = torch.zeros((1,), requires_grad=True, device='cuda:0')
